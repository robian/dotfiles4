...
local ok, err = xpcall(function()
  ...
  write(root .. "/.nvim.json", '{"web":{"formatter":"prettier"}}')
  eq(formatting.select(file, false).formatter, "prettier")
  write(root .. "/packages/app/.nvim.json", '{"web":{"formatter":false}}')
  eq(formatting.select(file, false).disabled, true)
  eq(formatting.select(file, true).disabled, true)
  write(root .. "/packages/app/.nvim.json", '{"web":{"formatter":"eslint"}}')
  fails(function()
    formatting.select(file, true)
  end, "web.formatter must be")
  write(root .. "/packages/app/.nvim.json", '{"web":{"formatter":"biome"}}')
  eq(formatting.select(file, false).formatter, "biome")
  local binary = root .. "/node_modules/.bin/prettier"
  write(binary, "#!/bin/sh\nexit 0")
  vim.fn.setfperm(binary, "rwx------")
  eq(web.new(file).command("prettier"), binary)
  ...
  eq(web.new(nested).dependency("tailwindcss"), false)
  eq(web.new(nested).command("prettier"), "prettier")
...
print("Web project policy: " .. count .. " assertions passed")
